...
from src.data.ingestion import verify_labels_schema, strip_exif_and_validate_image
...
def test_verify_labels_schema_success(temp_dir):
    ...
    df = pd.DataFrame({"image_id": ["img1.jpg", "img2.jpg"], "label": [0, 1]})
...
def test_verify_labels_schema_missing_column(temp_dir):
    ...
    df = pd.DataFrame({"image_id": ["img1.jpg"], "wrong_label_col": [0]})
    df.to_csv(csv_path, index=False)
...
def test_verify_labels_schema_null_values(temp_dir):
    ...
    df = pd.DataFrame({"image_id": ["img1.jpg", None], "label": [0, 1]})
    df.to_csv(csv_path, index=False)
