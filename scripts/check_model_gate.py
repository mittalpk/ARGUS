...
def check_model_performance(candidate_run_id: str, tracking_uri: str = "mlruns", threshold_pct: float = 2.0) -> bool:
    """
    ...
    try:
        ...
        if candidate_apcer is None:
            logger.error(f"Candidate run {candidate_run_id} has no metric 'val_apcer_at_1percent_bpcer'.")
            return False
    ...
    try:
        ...
        historical_runs = [r for r in runs if r.info.run_id != candidate_run_id and "val_apcer_at_1percent_bpcer" in r.data.metrics]
        
        if not historical_runs:
            logger.info("No historical baseline runs found for comparison. Promoting candidate run automatically.")
            return True
            
        best_historical_run = historical_runs[0]
        best_historical_apcer = best_historical_run.data.metrics["val_apcer_at_1percent_bpcer"]
        logger.info(f"Best historical baseline validation APCER: {best_historical_apcer:.4f} (Run ID: {best_historical_run.info.run_id})")
        ...
        logger.info(f"Maximum acceptable APCER threshold (with {threshold_pct}% relative tolerance): {max_acceptable_apcer:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--candidate_run_id", type=str, required=True, help="MLflow Run ID of the candidate model")
    parser.add_argument("--tracking_uri", type=str, default="mlruns", help="MLflow tracking URI")
    parser.add_argument("--threshold", type=float, default=2.0, help="Acceptable relative degradation threshold in %")
    
    args = parser.parse_args()
    success = check_model_performance(args.candidate_run_id, args.tracking_uri, args.threshold)
    if not success:
        sys.exit(1)
    sys.exit(0)
