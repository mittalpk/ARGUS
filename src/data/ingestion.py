...
def verify_labels_schema(csv_path: str) -> pd.DataFrame:
    ...
    if not required_cols.issubset(df.columns):
        raise ValueError(
            f"Schema validation failed. Missing required columns: {required_cols - set(df.columns)}"
        )

    # Check for empty labels
    if df["image_id"].isnull().any() or df["label"].isnull().any():
        raise ValueError(
            "Schema validation failed: Found null values in 'image_id' or 'label'."
        )
...
def run_ingestion(
    ...
    for idx, row in df.iterrows():
        ...
        if success:
            processed_records.append({"image_id": image_name, "label": row["label"]})
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        logger.error(
            "Usage: python ingestion.py <raw_dir> <processed_dir> <labels_csv>"
        )
        sys.exit(1)
    ...
    logger.info(
        f"Ingestion complete. Successfully processed: {len(df)} images. Failed/Corrupt: {len(fails)}"
    )
